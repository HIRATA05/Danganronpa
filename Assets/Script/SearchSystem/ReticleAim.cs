...
public class ReticleAim : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            ReticleMove(PlayerController.ReticleMode);
        }
        if (gameManager.playerController == PlayerController.MysteryMode)
        {
            ReticleMove(PlayerController.MysteryMode);
        }

    }

    private void ReticleMove(PlayerController mode)
    {
        //マウスの位置と照準器の位置を同期させる。
        ...
        if (Physics.Raycast(ray, out hit))
        {
            switch (mode)
            {
                case PlayerController.ReticleMode:
                    //探索可能オブジェクトに当たった時
                    ...
                    break;
                case PlayerController.MysteryMode:
